...
class SeqVec(xregs: HWRegPool, vvregs: HWRegPool, vpregs: HWRegPool, vsregs: HWRegPool, varegs: HWRegPool, vl: Int, cfg: Map[String, String]) extends InstSeq
{
  ...
  for(i <- 1 to vfnum)
  {
    ...
    val vf_instseq = new SeqSeq(shadow_vvregs, shadow_vpregs, vsregs, varegs, xregs, vec_mem, seqnum, mixcfg, use_mul, use_div, use_mix, use_fpu, use_fma, use_fcvt, use_amo, use_seg) //TODO: Enable configuration of enabling mul,div ops

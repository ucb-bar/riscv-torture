...
object Generator extends App
{
  ...
  def generate(confFile: String, outFileName: String): String = {
    ...
    generate(nseqs, memsize, fprnd, mix, vec, use_amo, use_mul, use_div, outFileName, run_twice)
  }
  ...
  def generate(nseqs: Int, memsize: Int, fprnd : Int, mix: Map[String,Int], veccfg: Map[String,String], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, outFileName: String, run_twice: Boolean): String = {
    ...
    val prog = new Prog(memsize, veccfg, run_twice)
    ProgSeg.cnt = 0
    SeqVec.cnt = 0
    val s = prog.generate(nseqs, fprnd, mix, veccfg, use_amo, use_mul, use_div, run_twice)

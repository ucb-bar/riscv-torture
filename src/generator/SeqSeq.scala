...
class SeqSeq(vregs: HWRegPool, pregs: HWRegPool, def_preg: Reg, sregs: HWRegPool, aregs: HWRegPool, xregs: HWRegPool, mem: Mem, nseqs: Int, mixcfg: Map[String,Int], use_mul: Boolean, use_div: Boolean, use_mix: Boolean, use_fpu: Boolean, use_fma: Boolean, use_fcvt: Boolean, use_amo: Boolean, use_seg: Boolean, use_stride: Boolean) extends VFInstSeq
{
  ...
  val name_to_seq = Map(
    ...
    "valu" -> (() => new SeqVALU(vregs, def_preg, sregs, use_mul, use_div, use_mix, use_fpu, use_fma, use_fcvt)), // TODO: Clean up
